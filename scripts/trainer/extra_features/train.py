...
if 'extra_mean' in metadata:
    normalization_stats['extra_features'] = {
        'mean':torch.FloatTensor(metadata['extra_mean']).to(device), 
        'std':torch.sqrt(torch.FloatTensor(metadata['extra_std'])**2 + noise_std**2).to(device),
    }
...
def prepare_data_from_tfds(data_path, split='train', is_rollout=False, batch_size=1):
    ...
    def prepare_rollout_inputs(context, features):
        ...
        if 'extra_features' in features:
            exfeat = tf.transpose(features['extra_features'], perm=[1, 0, 2])
            target_extra_features = exfeat[:, -1]
            out_dict['extra_features'] = exfeat[:, :-1]

        out_dict['n_particles_per_example'] = [tf.shape(pos)[0]]
        if 'step_context' in features:
            out_dict['step_context'] = features['step_context']
        out_dict['is_trajectory'] = tf.constant([True], tf.bool)
        return out_dict, target_position, target_extra_features
...
def main():
    ...
    simulator = Simulator(
        ...
        num_particle_types=2,
        particle_type_embedding_size=num_particle_types,
        device=device,
    )
