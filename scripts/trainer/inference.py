...
parser.add_argument("--metadata_path", help="metadata path", required=True, type=str)
parser.add_argument("--model", help="model file", required=True, type=str)
...
with open(os.path.join(metadata_path, 'metadata.json'), 'rt') as f:
...
def _read_metadata(data_path):
    with open(os.path.join(metadata_path, 'metadata.json'), 'rt') as fp:
...
def eval_single_rollout(simulator, features, num_steps, device):
    ...
    output_dict = {
        ...
        'global_context': features['step_context'].cpu().numpy() if features['step_context'] is not None else None ,
    }
...
def train(simulator):
    ...
    #     'model_state_dict': simulator.state_dict(),
    #     'optimizer_state_dict': optimizer.state_dict()
    # }, os.path.join(model_path, 'model' + str(step) + '.pth'))
    

def infer(simulator):
    ds = prepare_data_from_tfds(os.path.join(data_path, 'rollouts'), split='test', is_rollout=True)
    eval_rollout(ds, simulator, num_steps=num_steps, save_results=True, device=device, num_eval_steps=130)


def main():

    # Add material parameters in the input node
    node_extra = len(metadata['context_mean']) if 'context_mean' in metadata else 0

    simulator = Simulator(
        particle_dimension=args.dim,
        node_in=(INPUT_SEQUENCE_LENGTH+1) * args.dim + 16 + node_extra, # 7*2 = 14 + 16 = 30 for 2D
    ...
    ds = prepare_data_from_tfds(data_path, split='test', is_rollout=True)
    ...
    eval_rollout(ds, simulator, output_path, num_steps=num_steps, save_results=True, device=device, num_eval_steps=10)
